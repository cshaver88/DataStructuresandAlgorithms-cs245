...
public class TestMultiKeyList {
		...
		try {
			...
			for (int i = 0; i < data.length; i++) {
				if (!equal(data[i], L.get(i, keyIndex))) {
					System.out.println("Error in get at index " + i
							+ " for key " + keyIndex);
					pass = false;
				}
			}
	...
	public static boolean testIteratorBackward(TestData data[], MultiKeyList L,
			int keyIndex) {
		try {
			sortList(data, keyIndex);
			...
			while (it.hasPrevious()) {
				...
				if (!equal(data[arrayIndex], prev)) {
					System.out
							.println("Error in testIteratorBackward (key index "
									+ keyIndex
									+ ")  Mismatch at List index "
									+ arrayIndex);
					return false;
				}
				arrayIndex--;
			...
			if (arrayIndex != -1) {
				System.out.println("Error in testIteratorBackward  (key index "
						+ keyIndex + "):  iterator did not cover entire list");
				return false;
			}
		} catch (Exception e) {
			System.out.println("Error in testIteratorBackward  (key index "
	...
	public static TestData[] buildDeletedList(TestData data[],
			int indicesToDelete[]) {
	...
	public static boolean testDeleteIndex(TestData data[], MultiKeyList L,
			int keyIndex) {
		sortList(data, keyIndex);
	...
	public static boolean testDeleteKey(TestData data[], MultiKeyList L,
			int keyIndex) {
		sortList(data, keyIndex);
	...
	public static boolean testDeleteIterator(TestData data[], MultiKeyList L,
			int keyIndex) {
		sortList(data, keyIndex);
	...
	public static boolean testIteratorForward(TestData data[], MultiKeyList L,
			int keyIndex) {
		try {
			sortList(data, keyIndex);
			MultiKeyListIterator it = L.iterator(keyIndex);
			...
			while (it.hasNext()) {
				...
				if (!equal(data[arrayIndex], next)) {
					System.out
							.println("Error in testIteratorForward (key index "
									+ keyIndex + ")  Mismatch at List index "
									+ arrayIndex);
					return false;
				}
				arrayIndex++;
			...
			if (arrayIndex != data.length) {
				System.out.println("Error in testIteratorForward  (key index "
						+ keyIndex + "):  iterator did not cover entire list ("
	...
	public static void sortList(TestData data[], int keyIndex) {
		for (int i = 1; i < data.length; i++) {
			...
			while (insertIndex > 0
					&& data[insertIndex - 1].keys[keyIndex]
							.compareTo(insertElem.keys[keyIndex]) > 0) {
	...
	public static boolean forwardBacwardIterators(MultiKeyList L, int keyIndex) {
		...
		while (it.hasNext()) {
			...
			if (!equal(l1, l2)) {
				System.out
						.println("Moving iterator forward then backward should return the same element ...");
				return false;
			}
			it.next();
		}
	...
	public static boolean equal(TestData data, ListElem elem) {
		...
		for (int i = 0; i < elem.numKeys(); i++) {
			if (data.keys[i].compareTo(elem.key(i)) != 0) {
				System.out.println("Failure at key " + i + ":" + data.keys[i]
						+ "," + elem.key(i));
				return false;
			}
		}
		if (!data.data.equals(elem.data()))
			System.out.println("Failure at data " + data.data + ","
					+ elem.data());
	...
	public static void main(String[] args) {
		...
		for (int keyIndex = 0; keyIndex < NUM_KEYS; keyIndex++) {
			localTest = localTest
					&& testIteratorBackward(testData, L, keyIndex);
		}
		if (localTest) {
	...
	private static boolean testExceptions() {
		...
		try {
			for (int i = 0; i < SMALL_LIST + 1; i++)
				it.next();
			System.out
					.println("Should have thrown an exception for going past the list");
			success = false;
		...
		try {
			L.get(SMALL_LIST, 0);
			System.out
					.println("Should have thrown an exception for getting an illegal index");
		...
		try {
			...
			System.out
					.println("Should have thrown an exception for illegal index");
			success = false;
		...
		try {
			...
			it2.remove();
			System.out
					.println("Should have thrown an exception for removing before moving forward");
			success = false;
		} catch (IllegalStateException e) {

		} catch (Exception e) {
			System.out
					.println("Incorrect exception thrown for removing before moving forward "
							+ e);
			success = false;
		}
		...
		try {
			...
			it2.remove();
			System.out
					.println("Should have thrown an exception for removing twice in a row");
			success = false;
		} catch (IllegalStateException e) {

		} catch (Exception e) {
			System.out
					.println("Incorrect exception thrown for removing twice in a row "
							+ e);
			success = false;
		}
