...
public class DictionaryTest {
		...
		try {
				...
				if (nextStr.length() > 0) {
					...
					for (int i = 0; i <= nextStr.length(); i++) {
						if (!d.checkPrefix(nextStr.substring(0, i))) {
							passed = false;
							System.out.println("Failed to find prefix: "
									+ nextStr.substring(0, i) + " of "
									+ nextStr);
						}
					}
				}
			...
			for (int i = 0; i < badWords.length; i++) {
				...
				if (result.length != NUM_SUGGESTIONS) {
					System.out
							.println("Didn't get correct number of suggestions for "
									+ badWords[i]);
					System.out.println("  Expected " + NUM_SUGGESTIONS
							+ ", got " + result.length);
					passed = false;
				}
				for (int j = 0; j < result.length; j++) {
					...
					if (!d.check(result[j])) {
						System.out.println("Suggestion " + result[j]
								+ " not in dictionary.");
						passed = false;
					}
				}
			...
			for (int i = 0; i < goodWords.length; i++) {
				...
				if (result.length != 1) {
					System.out
							.println("Word "
									+ goodWords[i]
									+ " is in the dictionary -- suggest should return 1 item");
	...
	public static boolean smallTest() {
		...
		String words[] = { "cat", "cart", "dog", "apple", "ape", "breakfast",
				"breakneck", "queasy", "quash", "quail", "quick", "reason",
				"rickshaw", "reality" };

		String nonWords[] = { "carts", "ap", "break", "qu", "reasons",
				"buttercup", "under" };
		...
		for (int i = 0; i < nonWords.length; i++) {
			if (d.check(nonWords[i])) {
				System.out.println("Found word:" + nonWords[i]
						+ ", shoudn't have");
				passed = false;
			}
		}
	...
	public static void main(String args[]) {
		...
		if (!passedTests) {
			...
			System.out
					.println("Be sure to check output for tree printing & suggest by hand!");
